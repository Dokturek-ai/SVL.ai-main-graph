...
2. Content & Grounding:
  ...
  - Preserve every management-relevant qualifier EXACTLY as the source states it — dose modifiers (a loading / first-day / tapering dose, the total course length) and a therapy's line or rank (first-line vs reserve / alternative / second-line). If the source attaches such a qualifier through a footnote marker (e.g. `**`) or a reserve / alternative label, resolve it ONLY when its note text or label is itself present in the Context, and keep the qualifier; if that note is NOT in the Context, keep the marker verbatim (e.g. `azitromycin [**]`) and do NOT guess its meaning. Where the source explicitly ranks or reserves options, do not flatten them into an unqualified, co-equal list — but do not infer a ranking from bare typographic symbols alone.
