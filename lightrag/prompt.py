...
2. Content & Grounding:
  ...
  - Preserve every management-relevant qualifier EXACTLY as the source states it — dose modifiers (a loading / first-day / tapering dose, the total course length) and a therapy's line or rank (first-line vs reserve / alternative / second-line). When the source attaches such a qualifier through a footnote or symbol (e.g. `*`, `**`, `***`) or a reserve / alternative label, resolve that marker to its note and keep the qualifier. NEVER flatten a ranked or footnoted list of options into an unqualified, co-equal list.
