...
- **G4 versioned emit** — an immutable bundle `nodes.jsonl + edges.jsonl + manifest.json`; manifest = content hash over canonicalized sorted records + corpus list + pipeline pins + counts. Stable ids; a re-emit produces a computed delta; monotone ratchets (quarantine non-increasing, anchor coverage 100%, doc coverage non-decreasing, **orphan-rate non-increasing**). The manifest **counts include the orphan rate** (emitted nodes with degree 0) and edge/node ratio — a completeness signal surfaced, not hidden (measured live at ~12% on the 2026-07-08 sample; the promotion pass computes the exact full-graph number per emit).
- **G5 typing/normalization (in-repo half)** — a small controlled entity-type enum (validated at promotion) + deterministic name canonicalization + a versioned alias table. Prerequisites for G3 conflict-keys and G4 stable ids. **The enum names the clinical kinds the downstream linkage needs** — `Condition/Diagnosis`, `Medication/Drug`, `Symptom`, `Procedure`, `LabTest`, `Concept`, `Other` (~5–15) — so the emit signals which entities are diagnosis/drug resolution candidates. (This clinical typing is the domain layer the Fable blind-review could not design.)
...
### Cross-repo linkage — diagnoses + drugs (why the bundle carries `concept_ref`)

The guidelines graph stays **separate** — it never writes into the mkn10 graph. Linkage to diagnoses + drugs is a **contract, not a merge**, split across sibling briefs: mkn10 owns the resolver + vocabulary (`mention-concept-resolve-contract`: MKN-10 dg codes, SÚKL drugs, CUI); guidelines tags entities at ingest (`chunk-concept-ref-tagging`, open); mkn10 later harvests + reifies (`mkn10-svl-guidelines-harvest-source`, long-term "A"). Diagnoses resolve via `concept_ref.mkn10_code`, drugs via `concept_ref.cui`/SÚKL.

**This spec's part:** every emitted node carries an **optional `concept_ref = {mkn10_code?, cui?}`** propagated from the chunk-concept-ref-tagging pass (null when untagged → mkn10 resolves at harvest, from the typed canonical form). The promotion pass **propagates**, it does **not** resolve — resolution stays mkn10's single source of truth. This makes the bundle the "grounded artifact" the harvest-source brief consumes.
